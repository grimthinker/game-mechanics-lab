...
import { BTAction, BTDecorator, BTNode, EntityContext, NodeStatus } from "./core";


export abstract class BTCondition extends BTDecorator {
    public static readonly nodeName = "Условие";
    public static readonly description = "Если условие не выполняется, возвращает FAILURE, иначе передает управление дочернему узлу";

    private condition: (ctx: EntityContext) => boolean;

    constructor(condition: (ctx: EntityContext) => boolean, child: BTNode) {
        super(child);
...
export class BTInverter extends BTDecorator {
    ...
    public static readonly description = "Инвертирует статус выполнения дочернего узла: SUCCESS меняет на FAILURE, FAILURE на SUCCESS";

    constructor(child: BTNode) { super(child); }
...
export class BTRetry extends BTDecorator {
    ...
    public static readonly description = "Повторяет выполнение дочернего узла при неудаче до тех пор, пока он не вернет SUCCESS";
...
export class BTCooldown extends BTDecorator {
    ...
    public static readonly description = "Блокирует повторное выполнение дочернего узла на заданное время (cooldownMs)";
...
export class BTRepeater extends BTDecorator {
    ...
    public static readonly description = "Бесконечно повторяет выполнение дочернего узла, игнорируя его завершение";
...
export class BTDecoratorCheckEngaged extends BTDecorator {
    ...
    public static readonly description = "Проверяет, находится ли цель на расстоянии ближе или равном engage_dist единицам, иначе прерывает дочерний узел";
    ...
    protected onTick(ctx: EntityContext): NodeStatus {
        ...
        if (!target?.control.get_pos()) return NodeStatus.FAILURE;

        const dist = vec2_distance_to(ctx.control.get_pos(), target.control.get_pos());
...
export class BTDecoratorIsTargetAlive extends BTDecorator {
    ...
    public static readonly description = "Проверяет наличие и статус жизни цели, прерывая выполнение при ее гибели";
